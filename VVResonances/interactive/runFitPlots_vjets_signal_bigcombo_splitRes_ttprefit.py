...
if __name__=="__main__":
     ...
     if options.addTop==True:
        ...
        workspaceTT = fTT.Get("w")
        workspaceTT.var("MH").setVal(options.signalMass)
        workspaceTT.var("MH").setConstant(1)
        fTT.Close()
        workspaceTT.Print()                                                                                                                                                                                                                  
        modelTT = workspaceTT.pdf("model_b")
        model_bTT = workspaceTT.pdf("model_b")
        data_TT = workspaceTT.data("data_obs")
        argsTT  = modelTT.getComponents()
        data_TT.Print()
        dataTT = {}
        pdf1NameTT = {}
        TT_expected = {}
        ...
        for year in years:
            ...
            print dataTT[year].sumEntries() ,"   ("+year+")"
            expectedTT = {}
            N_expected_TT = 0
            for bkg in TTbkgs:
                expectedTT[bkg] = [ (argsTT[pdf1NameTT[year]].getComponents())["n_exp_binJJ_"+purity+"_13TeV_"+year+"_proc_"+bkg],0.]
                print "Expected number of "+bkg+" events:",(expectedTT[bkg][0].getVal()),"   ("+year+")"
                N_expected_TT += (expectedTT[bkg][0].getVal())
                TT_expected[year] = expectedTT 
                signal_TTexpected[year] = [0.,0.]

        print 
    
        print "Total number of expected TT background events:",N_expected_TT,"--> data/bkg =",dataTT[year].sumEntries()/N_expected_TT,"(",year,")"
        ################################################# do the fit ###################################
        print
        
        if options.fit:
            fitresultTT = modelTT.fitTo(data_TT,ROOT.RooFit.SumW2Error(not(options.data)),ROOT.RooFit.Minos(0),ROOT.RooFit.Verbose(0),ROOT.RooFit.Save(1),ROOT.RooFit.NumCPU(8))  
            fitresultTT.Print() 
        print 
        writeLogfile(options,fitresultTT)
        ############################################################################################
        ...
        for year in years:
            TTpdfs[year] = []
            for bkg in TTbkgs:
                TTpdfs[year].append(argsTT["shapeBkg_"+bkg+"_JJ_"+purity+"_13TeV_"+year])
            ...
            print "Full "+year+" TT post-fit pdf:"     
            TTpdfs[year].append( argsTT[pdf1NameTT[year]+"_nuis"])
            TTpdfs[year][-1].Print()
            TTsignalpdfs[year] =None
        ...
        for year in years:
            ...
            for bkg in TTbkgs:
                print
                (argsTT[pdf1NameTT[year]].getComponents())["n_exp_binJJ_"+purity+"_13TeV_"+year+"_proc_"+bkg].dump()
                expectedTT[bkg] = [ (argsTT[pdf1NameTT[year]].getComponents())["n_exp_binJJ_"+purity+"_13TeV_"+year+"_proc_"+bkg],(argsTT[pdf1NameTT[year]].getComponents())["n_exp_binJJ_"+purity+"_13TeV_"+year+"_proc_"+bkg].getPropagatedError(fitresultTT)]
                print "normalization of "+bkg+" after TT fit:",(expectedTT[bkg][0].getVal()), " +/- ",expectedTT[bkg][1] ,"   ("+year+")"
            TT_expected[year] = expectedTT  
        print " dumped tt pdf " 
	...
	N_expected_tot = 0
     ...
     print " calling forproj with hinMC ",hinMC
