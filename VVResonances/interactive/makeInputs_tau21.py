...
#examples:
# python makeInputs_tau21.py -p 2016 --run "signorm" --signal "ZprimeWW" --batch False 
# python makeInputs_tau21.py -p 2016 --run "vjets" --batch False                                                                                                                                      
# python makeInputs_tau21.py -p 2016 --run "qcdtemplates"
# python makeInputs_tau21.py -p 2016 --run "qcdkernel"
# python makeInputs_tau21.py -p 2016 --run "qcdnorm"
# python makeInputs_tau21.py -p 2016 --run "data"
parser = OptionParser()
...
if sorting == 'random':
 ...
 cuts['VH_HPLP'] = '(' + '('+  '&&'.join([catVtag['HP1'],catHtag['LP2']]) + ')' + '||' + '(' + '&&'.join([catVtag['HP2'],catHtag['LP1']]) + ')' + ')' 
 #cuts['VH_LPHP'] = '(' + '('+  '&&'.join([catVtag['LP1'],catHtag['HP2']]) + ')' + '||' + '(' + '&&'.join([catVtag['LP2'],catHtag['HP1']]) + ')' + ')' #old
...
# cuts['VH_LPLP'] = '(' + '('+  '&&'.join([catVtag['LP1'],catHtag['LP2']]) + ')' + '||' + '(' + '&&'.join([catVtag['LP2'],catHtag['LP1']]) + ')' + ')' #old
...
# print " ################ VV orthogonal  VH CATEGORY ############" 
 #old Anna cuts['VV_HPHP'] = '('+'!'+cuts['VH_all']+'&&'+'('+'('+'('+'&&'.join([catVtag['HP1'],catVtag['HP2']])+ ')'+ '||'+ '('+'&&'.join([catVtag['HP1'],catHtag['HP2']])+')'+')'+ '||' + '('+'(' + '&&'.join([catVtag['HP1'],catVtag['HP2']])+')'+'||'+'('+ '&&'.join([catHtag['HP1'],catVtag['HP2']])+')' +')'+ ')' +')' #Anna's
...
if options.run.find("all")!=-1 or options.run.find("qcd")!=-1:
    ...
    if runParallel and submitToBatch:
        ...
        f.makeBackgroundShapesMVVConditional("nonRes","JJ_"+str(period),nonResTemplate,'l2',cuts['nonres'],"2Dl2",wait)
    if options.run.find("all")!=-1 or options.run.find("norm")!=-1:
        print "merging background shapes"
        f.mergeBackgroundShapes("nonRes","JJ_"+str(period))
        print "background norm"
